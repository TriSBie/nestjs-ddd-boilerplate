...
export class RequestMiddleware implements NestMiddleware {
    ...
    use(request: Request, response: Response, next: NextFunction): void {
        ...
        // Listen to the finish event of the response
        // This event is emitted when the response is finished sending to the client.
        response.on('finish', () => {
        ...
        response.on('error', error => {
            this.logger.error(
                `✖ ${method} ${originalUrl} - Request failed [${requestId}]`,
                error
            );
        });
